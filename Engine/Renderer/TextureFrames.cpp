...
namespace nu {
	...
	bool TextureFrames::Load(const std::string& filename, class Renderer& renderer) {
		...
		if (!json::Load(filename, document)) {
			std::cerr << "Couldn't load Texture Frames:" << filename << std::endl;
			return false;
		}
		...
		if (!m_texture) {
			std::cerr << "Couldn't load Texture Frame texture: " << texture_name << std::endl;
			return false;
		}
		...
		if (m_numColumns <= 0 || m_numRows <= 0) {
			std::cerr << "Texture Frame rows or columns is either 0 or less than 0" << std::endl;
			return false;
		}
	...
	Rect TextureFrames::GetFrameRect(unsigned int frame) {
		if (frame < 0 || frame >= m_totalFrames) {
			std::cerr << "Texture Frames frame " << frame << " is out of bounds. Starting at beginning frame.\n";
			frame = 0;
		}
